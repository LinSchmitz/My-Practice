...
function hitAlien() {
  ...
  const audio = new Audio('Thump.mp3'); // make sure file exists
  audio.volume = 0.2;
  audio.play();

  // Change alien emoji to poop
  alien.textContent = '💩';

  // Trigger explode animation
  alien.classList.add('explode');

  // After animation, reset or hide alien
  alien.addEventListener(
    'animationend',
    () => {
      alien.classList.remove('explode');
      alien.classList.add('hidden');
      alien.textContent = '👳🏼‍♂️'; // reset back to original for next spawn
    },
    { once: true },
  );
}
